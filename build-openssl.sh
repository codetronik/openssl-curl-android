...
./Configure android-arm64 no-shared no-debug no-apps no-autoerrinit --prefix=$PWD/build/$ANDROID_ARCH --release
...
./Configure android-arm no-shared no-debug no-apps no-autoerrinit --prefix=$PWD/build/$ANDROID_ARCH --release
